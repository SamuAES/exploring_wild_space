...
def extract_1(json_path, video_id, frame_count: int = np.inf, w: int = 3):
    fps=30

    df_perches, df_movement, df_sections = extract_from_json(json_path, video_id, frame_count, fps)
    #df_raw = pd.read_csv(f"../../data_yolo_nano_v2/{video_id}_exploration_IB.csv")[:frame_count]
    # Apply data imputation and sliding average
    df = preprocess_data(df_movement, w)
    # Compute total travelled distance
...
def extract_from_json(json_path, video_name, nframes, fps):

    raw_data = load_json_to_dict(json_path)
    #bird_df, wall_df, fence_df, perch_df = process_raw_data(raw_data)

    if nframes > len(raw_data):
    ...
    for ind, frame_name in enumerate(raw_data):
        
        if ind == nframes:
            break

        fr = raw_data[frame_name]
        frame = {}
    ...
    smoothed_perching_result = smooth_bird_action(perching_result)

    for i in range(len(smoothed_perching_result)):
        # Computation of 5perches:
        # Remove perch number from reference set (does nothing if number is not present)
        five_perches_set.discard(smoothed_perching_result[i])
        ...
        if len(five_perches_set) > 0:
            ...
            if bird_x < all_wall_xs[i]:
                five_perches_timer += 1

    # Compute features. Boris helpdoc only asks for perches 1-5, but here are 6-8 as well
    perch1, perch2, perch3, perch4, perch5, perch6, perch7, perch8 = compute_perch_durations(smoothed_perching_result, fps)
    ground, fence = compute_ground_and_fence(smoothed_perching_result, fps)
...
json_path = './data/raw_data/CAGE_220520_HA70339_exploration_IB.json'
video_name = 'CAGE_220520_HA70339_exploration_IB'


res = extract_1(json_path,video_name,frame_count)
res.to_csv(f"data/features/{video_id}_testi1_1.csv", header = True, index = False)